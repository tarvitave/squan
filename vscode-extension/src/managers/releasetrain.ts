...
import type { ReleaseTrain } from '../types'
...
export const releaseTrainManager = {
  async create(
    ...
    await db.execute({
      sql: `INSERT INTO release_trains (id, name, rig_id, atomic_task_ids_json, description, status, manual, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, 'open', ?, ?, ?)`,
      args: [id, name, rigId, JSON.stringify(atomicTaskIds), description ?? '', manual ? 1 : 0, now, now],
    })
  ...
  async listByProject(projectId: string): Promise<ReleaseTrain[]> {
    const db = getDb()
  ...
  async start(releaseTrainId: string): Promise<void> {
    const db = getDb()
    await db.execute({
  ...
  async land(releaseTrainId: string): Promise<void> {
    const db = getDb()
    await db.execute({
  ...
  async cancel(releaseTrainId: string): Promise<void> {
    const db = getDb()
    await db.execute({
  ...
  async moveToPrReview(releaseTrainId: string, prUrl: string, prNumber: number): Promise<ReleaseTrain> {
    const db = getDb()
    await db.execute({
